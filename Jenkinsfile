...
node('rhel7'){
	...
	if(params.UPLOAD_LOCATION) {
		stage('Upload') {
			...
			sh "rsync -Pzrlt --rsh=ssh --protocol=28 ${filesToPush[0].path} ${UPLOAD_LOCATION}/snapshots/intellij-mta/"
    ...
    if(publishToMarketPlace.equals('true')){
        timeout(time:5, unit:'DAYS') {
        	input message:'Approve deployment?', submitter: 'johnsteele'
    	}
    	...
    	stage("Publish to Marketplace") {
            ...
            if (!isSnapshot) {
                stage("Promote the build to stable") {
                    ...
                    sh "rsync -Pzrlt --rsh=ssh --protocol=28 ${zip[0].path} ${UPLOAD_LOCATION}/stable/intellij-mta/"
                }
            }
        }
    }
